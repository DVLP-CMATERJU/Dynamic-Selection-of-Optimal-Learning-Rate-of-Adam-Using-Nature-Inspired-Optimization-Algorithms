# Generated from: lc25000-sgm-id.ipynb
# Converted at: 2026-06-26T11:25:51.065Z
...
# === PATHS ===
root_dir = "/kaggle/input/datasets/javaidahmadwani/lc25000/lung_colon_image_set"
test_dir = os.path.join(root_dir, "Test Set")
train_val_dir = os.path.join(root_dir, "Train and Validation Set")

# Classes you want
classes = ["colon_aca", "colon_n"]

# Output folder
os.makedirs("/kaggle/working/lc25000_colon", exist_ok=True)
output_dir = "/kaggle/working/lc25000_colon"
...
def copy_images(src_folder, dst_folder):
    for img_name in os.listdir(src_folder):
        ...
        # If duplicate filename exists → rename
...
for cls in classes:
    ...
    train_cls_path = os.path.join(train_val_dir, cls)

    # Destination
    dst_cls_path = os.path.join(output_dir, cls)

    # Copy from both
    copy_images(test_cls_path, dst_cls_path)
    copy_images(train_cls_path, dst_cls_path)
...
# ================= DATA =================
data_path = "/kaggle/working/lc25000_colon"
...
model = Model1(2).float().to(device)
checkpoint = torch.load('/kaggle/input/datasets/shreyanmajumdar/lc25000-arabian-ckpts/best_NO_OPT_run4 (2).pth', weights_only = True)
print(checkpoint.keys())
model.load_state_dict(checkpoint)
model.eval()
...
model = Model1(2).float().to(device)
checkpoint = torch.load('/kaggle/input/datasets/shreyanmajumdar/lc25000-arabian-ckpts/best_GWO_run4 (2).pth', weights_only = True)
print(checkpoint.keys())
model.load_state_dict(checkpoint)
model.eval()
...
model = Model1(2).float().to(device)
checkpoint = torch.load('/kaggle/input/datasets/shreyanmajumdar/lc25000-arabian-ckpts/best_FIREFLY_run4 (2).pth', weights_only = True)
print(checkpoint.keys())
model.load_state_dict(checkpoint)
model.eval()
...
model = Model1(2).float().to(device)
checkpoint = torch.load('/kaggle/input/datasets/shreyanmajumdar/lc25000-arabian-ckpts/best_COVID_19_run4 (2).pth', weights_only = True)
print(checkpoint.keys())
model.load_state_dict(checkpoint)
model.eval()
...
model = Model1(2).float().to(device)
checkpoint = torch.load('/kaggle/input/datasets/shreyanmajumdar/lc25000-arabian-ckpts/best_GSA_run4 (2).pth', weights_only = True)
print(checkpoint.keys())
model.load_state_dict(checkpoint)
model.eval()
...
model = Model1(2).float().to(device)
checkpoint = torch.load('/kaggle/input/datasets/shreyanmajumdar/lc25000-arabian-ckpts/best_PSO_run4 (2).pth', weights_only = True)
print(checkpoint.keys())
model.load_state_dict(checkpoint)
model.eval()
...
model = Model1(2).float().to(device)
checkpoint = torch.load('/kaggle/input/datasets/shreyanmajumdar/lc25000-arabian-ckpts/best_AVERAGE_run4 (2).pth', weights_only = True)
print(checkpoint.keys())
model.load_state_dict(checkpoint)
model.eval()
