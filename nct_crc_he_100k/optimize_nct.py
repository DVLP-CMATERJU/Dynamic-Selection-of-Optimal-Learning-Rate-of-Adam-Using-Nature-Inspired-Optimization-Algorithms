...
# ================= OPT =================
OPT = "gwo"   # gwo | pso | firefly | gsa | c19boa
...
# =========================================
# 📦 DATA (YOUR EXACT BLOCK — UNCHANGED)
# =========================================
...
# ================= K-FOLD SETUP (for LR search only) =================
K_FOLDS = 5
kfold = KFold(n_splits=K_FOLDS, shuffle=True, random_state=42)
# train_full_dataset is the pool used for LR-search fitness evaluation
fold_splits = list(kfold.split(np.arange(len(train_full_dataset))))
...
# ================= TRAIN (single fold) =================
def train_one_fold(lr, fold_train_loader, fold_val_loader):
...
# ================= TRAIN (5-fold CV wrapper, used during LR search) =================
def train_one_lr(lr, tag):
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(fold_splits):
        ...
        fold_train_subset = Subset(train_full_dataset, tr_idx)
        fold_val_subset    = Subset(train_full_dataset, va_idx)

        fold_train_loader = DataLoader(fold_train_subset, batch_size=128, shuffle=True, num_workers=2)
        fold_val_loader   = DataLoader(fold_val_subset, batch_size=128, shuffle=False, num_workers=2)

        fold_acc = train_one_fold(lr, fold_train_loader, fold_val_loader)
        fold_accs.append(fold_acc)

        with open(CSV_PATH, "a") as f:
            f.write(f"{OPT},{tag},fold{fold_idx},{lr},{fold_acc}\n")

    return float(np.mean(fold_accs))   # ✅ ONLY CHANGE (val objective = 5-fold mean)

# ================= FINAL TEST (single train/val split — unchanged) =================
def test_best_model(best_lr):
    ...
    best_val_acc = 0
    best_path = f"{SAVE_DIR}/final_best.pth"
...
final_test_acc = test_best_model(best_lr)
